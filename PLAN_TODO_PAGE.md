...
- [ ] `packages/shared-types/src/app.ts`에서 PageType에 "todos" 추가
- [ ] `apps/frontend/src/app/todos/page.tsx` 기본 페이지 생성
- [ ] `apps/frontend/src/components/sidebar.tsx`에 할 일 메뉴 아이템 추가
- [ ] CheckSquare 아이콘 import 및 적용
- [ ] 메뉴 순서 조정 (홈 → 할 일 → 통계 → 설정)
- [ ] 기본 라우팅 테스트

### 2단계: 핵심 컴포넌트 구현
- [ ] `apps/frontend/src/components/todos/` 디렉토리 생성
- [ ] `TodosPage.tsx` 기본 컴포넌트 생성
- [ ] AppLayout과 PageHeader 적용
- [ ] withAuth HOC 적용
- [ ] 기본 할 일 목록 표시 기능 구현
- [ ] useTodoContext를 사용한 데이터 연동
- [ ] 기본 스타일링 적용

### 3단계: 필터링 및 검색 기능
- [ ] `TodosFilter.tsx` 컴포넌트 생성
  - [ ] 완료 상태 필터 (전체/완료/미완료)
  - [ ] 할 일 타입 필터 (전체/이벤트/작업)
  - [ ] 카테고리 다중 선택 필터
  - [ ] 날짜 범위 필터
- [ ] `TodosSearch.tsx` 컴포넌트 생성
  - [ ] 실시간 검색 기능
  - [ ] 검색 결과 하이라이트
- [ ] `useTodosPage.ts` 훅 생성
  - [ ] 필터 상태 관리
  - [ ] 검색어 상태 관리
  - [ ] 로컬 스토리지 연동
- [ ] 필터 및 검색 로직 통합
...
### 4단계: 고급 기능 및 UX 개선
- [ ] `TodosView.tsx` 컴포넌트 생성
  - [ ] 목록 뷰 / 카드 뷰 전환
  - [ ] 정렬 옵션 구현 (날짜순, 카테고리순, 완료순)
- [ ] 할 일 관리 기능 통합
  - [ ] 인라인 편집 기능
  - [ ] 완료/미완료 토글
  - [ ] 삭제 기능
